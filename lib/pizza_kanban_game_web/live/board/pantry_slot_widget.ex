    ...
    ~H"""
      <div class="relative flex flex-col items-center">
        ...
        <div :if={{@game && @game.clock > 0}}
          class="z-auto absolute -top-1 -left-4 text-xs bg-blue-600 w-4 text-center text-white rounded-full ">
          {{@slot.quantity}}
        </div>
        <div :if={{@game && @game.clock > 0}}
          class="z-auto absolute -top-1 -right-4 text-xs bg-red-600 w-4 text-center text-white rounded-full "
              :on-click="inc">
          +
        </div>
